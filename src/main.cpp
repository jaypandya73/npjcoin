...
#if defined(NDEBUG)
# error "Npjcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x32c6f4be27d312ed0ac333d638b13030ab49c21661d1ad00d9a6e5371ae79843");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Npjcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Npjcoin Signed Message:\n";
...
{
    // Npjcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Npjcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Npjcoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Npjcoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Npjcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Npjcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xed9a1c651f114c60e48f964e92e91cecef0b3b68f39772f60afd5f22724374d6");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is so cool man";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042cecdbd7ad651f5f056211f86e8ad35056ba78358c20e857d92990374330d883ddc4789852b8a58aab88654e8be335a062b55ab249e8c7de7f445e6a51340830") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517598819;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085675397;

        if (fTestNet)
        {
            block.nTime    = 1517598796;
            block.nNonce   = 387279945;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x40d4403291e65bc3ca4dc88e76e24291de49cef4ca22c16036631099a7619c5b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Npjcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NpjcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NpjcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NpjcoinMiner(CWallet *pwallet)
{
    printf("NpjcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("npjcoin-miner");
    ...
    try { loop {
        ...
        printf("Running NpjcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NpjcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NpjcoinMiner, pwallet));
}
